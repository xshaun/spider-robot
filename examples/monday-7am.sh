...
# 1. crontab -e
#      # 每周一 早6:58分 开始运行脚本
#      58 6 * * 1 /root/monday-7am.sh > ./monday-log-7.00.txt
#      # 每周一 早7:18分 再次运行脚本
#      18 7 * * 1 /root/monday-7am.sh > ./monday-log-7.20.txt
# 2. crontab -l
#
...
# stall_weeks=4%2C1%2C982%2C0%2C1208%2C2 映射 4 weekends with electricity $1208
# stall_weeks=4%2C0%2C982%2C0%2C1088%2C2 映射 4 weekends $1088.00

echo -e "\n\n--rush--"
arr=(
    "4%2C1%2C982%2C0%2C1208%2C2"

    "4%2C0%2C982%2C0%2C1088%2C2"
...
for i in ${arr[@]}; do
  ...
  while [[ $recode -eq 0 ]]; do
    ...
    echo -e "\n\n--You have already booked and paid for a stall--"
    curl -A "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.0)" -b cookie1.txt -X POST --data "bFormSubmitted=yes&tenant_id_pay=&stall_weeks=$i&dates_1=%7Bd+%27${DATES_1}%27%7D&dates_2=%7Bd+%27${DATES_2}%27%7D&Submit=Finish" "${HOSTURL}/login_register.cfm" | grep "You have already booked and paid for a stall"
    if [[ $? -eq 0 ]]; then
      break;
    fi
